...
6.7.8 - 2026年1月16日
- 支援SQLite version 3.51.2 (2026-01-09)。
- 其它問題修復與穩定性改善。

6.7.7 - 2025年12月24日
- 資料編輯器。 資料編輯器的預設字型大小現在將是標準系統字型大小。
- 資料庫列表。 顯示資料庫位置的預設值現在為真。
- 將SQLCipher更新到4.12.0。
- 額外的錯誤修復和穩定性改進。
...
6.7.4 - 2025年11月6日
- 在「設置」-「通用」中，支持顯示SQLite庫的日期（來自sourceid）。
- 修復了一個如果虛擬表上存在無法識別的令牌化器，導致數據庫加載失敗的問題。
- 內部查看器。修復可能導致應用程序崩潰的問題。
- 內部查看器。支持在溢出頁面上顯示更多詳細信息。
- 支援SQLite version 3.51.0 (2025-11-04)。
- 其它問題修復與穩定性改善。
